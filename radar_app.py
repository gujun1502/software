...
全程不需要 VPN，不依赖 Claude。大模型按 llm_config.json 自动启用（本地 Ollama 优先，MiniMax 兜底）。
"""
import sys
...
def main():
    ...
    if cmd in ("report", "radar", "daily", "日报"):
    ...
    elif cmd in ("update", "auto"):
        def _evolve():
            sys.argv = [argv0]; import evolve; evolve.main()
        if cmd == "auto":
            print("=== 每日自动全流程：进化 → 刷新 → 日报 ===\n")
            _safe("每日进化", _evolve)
        ...
        def _reg():
            sys.argv = [argv0]; import fetch_registry; fetch_registry.main()
        def _intent():
            sys.argv = [argv0]; import fetch_intention; fetch_intention.main()
        def _enrich():
        ...
        _safe("上海抓取", _sh)
        _safe("注册表多源抓取", _reg)
        _safe("采购意向抓取", _intent)
        _safe("详情页增强", _enrich)
        sys.argv = [argv0, "--open"] if cmd == "update" else [argv0]
        import run_radar
        run_radar.main()
